...
const ACCENT_MAP = { Ocean: WL.ocean, Sun: WL.sun, Lime: WL.lime, Aqua: WL.aqua };
...
// ─── 사인파 path ───
function makeWavePath(time, opts) {
...
function MorphingWave({ scrollPx = 0, color1 = WL.ocean, color2 = WL.aqua, height = 80, flip = false }) {
  const time = useTime();
...
// ─── 섹션 라벨 ───
function IssueLabel({ num, label, accent = WL.ink }) {
  return (
    <div style={{
...
// ─── 한글 헤드라인 (Black Han Sans) ───
function HanHead({ children, color = WL.ink, size = 60, line = 1.0, style = {} }) {
  return (
    <div style={{
      fontFamily: FF.han,
      fontSize: size,
...
// ═════════════════════════════════════════════════════════
// 04 · TIMELINE — 그룹 디바이더 + 컬러 배경
// ═════════════════════════════════════════════════════════
const TIMELINE_GROUP_TONE = {
...
function Timeline({ scrollPx }) {
  ...
  // 섹션 진행도 — 섹션 top이 뷰포트 top에 닿으면 0, 섹션 bottom이 뷰포트 bottom에 닿으면 1
  ...
  return (
    <section ref={sectionRef} data-screen-label="04 Timeline" style={{
      ...
      <Reveal delay={0.1}>
        <div style={{ marginTop: 24 }}>
          <HanHead size={56} line={0.95} color={fg}>
            하루 종일,
          </HanHead>
          <HanHead size={56} line={0.95} color={WL.ocean} style={{ fontStyle: 'italic' }}>
            함께해요.
          </HanHead>
        </div>
      </Reveal>
...
// ═════════════════════════════════════════════════════════
// 05 · PROGRAM — 3행 드래그 캐러셀 + 모달 확장
// ═════════════════════════════════════════════════════════
function ProgramCard({ card, accent, onClick, dimmed }) {
  return (
    <div
      onClick={onClick}
      style={{
...
function ProgramRow({ row, onCardClick, paused, expandedKey }) {
  const accent = ACCENT_MAP[row.accent] || WL.ocean;
  const trackRef = useRef(null);
  const offsetRef = useRef(0);          // 현재 translateX (px)
  const lastTimeRef = useRef(0);
  const halfWidthRef = useRef(0);       // 한 세트 너비 (= 전체/2)
  const [dragging, setDragging] = useState(false);
  const draggingRef = useRef(false);    // rAF 즉시 반영용 (state는 한 프레임 늦음)
  const dragStateRef = useRef({ startX: 0, startOffset: 0, lastX: 0, lastT: 0, vel: 0, moved: 0, pointerDown: false });
  const inertiaRef = useRef(0);         // 드래그 후 관성 속도 (px/ms)
  const DRAG_THRESHOLD = 6;             // 이 이상 움직여야 드래그로 인식

  // offset을 [-hw, 0) 범위로 정규화 — 무한 루프 핵심
  const wrap = (x) => {
    const hw = halfWidthRef.current;
    if (hw <= 0) return x;
    // ((x % hw) + hw) % hw → [0, hw), 그 다음 -hw 더해서 [-hw, 0)
    return (((x % hw) + hw) % hw) - hw;
  };

  // 카드 1개 132 + gap 10 = 142
  const CARD_STRIDE = 142;
  const direction = row.direction === 'right' ? 1 : -1;
  // duration(초)에 한 세트(halfWidth)를 가로지름 → 속도 = halfWidth/duration (px/s) → /1000 (px/ms)
  ...
  // rAF 루프 — paused/dragging 아닐 때만 자동 진행, 드래그 후 관성 적용
  useEffect(() => {
    let rafId;
    const tick = (t) => {
      ...
      if (!paused && !draggingRef.current && halfWidthRef.current > 0) {
        // 관성 속도 + 기본 속도
        const inertiaFade = Math.exp(-dt / 250); // 250ms 시정수
  ...
  // 포인터: down에선 드래그 시작 안 함 — 임계값 넘는 move부터 드래그로 전환
  ...
  const onPointerMove = (e) => {
    ...
    st.vel = (e.clientX - st.lastX) / dt; // px/ms
    st.lastX = e.clientX;
    st.lastT = now;
...
function ProgramModal({ data, onClose }) {
  // ESC 닫기
  useEffect(() => {
...
function Program({ scrollPx }) {
  const [openCard, setOpenCard] = useState(null); // { card, accent, rowLabel, rowKey }

  const handleCardClick = (rowKey, cardId) => {
    const row = D.programRows.find(r => r.key === rowKey);
    if (!row) return;
    const idx = parseInt(cardId.split('-').pop(), 10);
        ...
        <div style={{
          ...
          {D.programRows.map((row) => (
            <ProgramRow
              ...
              expandedKey={null}
              onCardClick={(cardId) => handleCardClick(row.key, cardId)}
            />
          ))}
        </div>
      </Reveal>
...
// 06 · FOOD — 점심 / 간식 분리
// ═════════════════════════════════════════════════════════
function FoodBlock({ d, accent, blue = false }) {
  return (
    <div style={{
...
// ═════════════════════════════════════════════════════════
// 07 · DIRECTIONS — 지도 placeholder + 셔틀 + 주차
// ═════════════════════════════════════════════════════════
function Directions({ scrollPx }) {
  return (
...
// ═════════════════════════════════════════════════════════
// 08 · GALLERY — 사진 placeholder
// ═════════════════════════════════════════════════════════
function Gallery({ scrollPx }) {
  return (
...
// ─── 디바이스 래퍼 ───
function ResponsiveDevice({ children, dark = false }) {
